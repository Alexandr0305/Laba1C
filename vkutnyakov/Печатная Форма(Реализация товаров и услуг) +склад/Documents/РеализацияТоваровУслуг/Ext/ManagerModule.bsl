...
Процедура Расш1_ДобавитьКомандыПечати(КомандыПечати)
		...
		КомандаПечати.Идентификатор = "Накладная_Доработка";
		КомандаПечати.Представление = НСтр("ru = '--Реализация товаров Кутняков В.'");
...
Процедура Расш1_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) 
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная_Доработка") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"Накладная_Доработка",
			НСтр("ru = '111Реализация товаров'"),
			СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати),
			,
			"ПФ_MXL_РеализацияТоваровКутняков");
		КонецЕсли;
КонецПроцедуры


&Вместо("СформироватьПечатнуюФормуНакладная")
Функция Расш1_СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|   ВложенныйЗапрос.Ссылка.Склад 								КАК Склад
	|ИЗ
	|	(ВЫБРАТЬ
	|		Таблица.Ссылка					КАК Ссылка,
	|		ВЫБОР
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_РеализацияТоваровКутняков");
