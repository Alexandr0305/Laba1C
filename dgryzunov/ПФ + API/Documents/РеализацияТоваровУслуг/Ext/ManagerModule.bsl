...
Процедура ОР_ДобавитьКомандыПечатиПосле(КомандыПечати) Экспорт 
	...
	Если ЭтоПартнер ИЛИ ПолучитьФункциональнуюОпцию("НеВыбиратьВариантВыводаСкидокПриПечатиДокументовПродажи") Тогда
		// Реализация товаров со складом
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "НакладнаяСоСкладом";
		КомандаПечати.Представление = НСтр("ru = 'Реализация товаров Грызунов Д.В.'");
...
&После("Печать")
Процедура ОР_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяСоСкладом") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"НакладнаяСоСкладом",
			НСтр("ru = 'Реализация товаров Грызунов Д.В.'"),
			ОР_СформироватьПечатнуюФормуНакладнаяСоСкладом(МассивОбъектов, ОбъектыПечати, ПараметрыПечати),
			,
...
Функция ОР_СформироватьПечатнуюФормуНакладнаяСоСкладом(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Запрос.УстановитьПараметр("Ссылка", ДанныеПрослеживаемостиДокументов[0].Ссылка);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписиСклад = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаДетальныеЗаписиСклад.Следующий() Цикл
		СкладВДокументе = ВыборкаДетальныеЗаписиСклад.Склад;
	КонецЦикла;   
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		НазванияОбластейКолонок = ДобавитьКолонкуСклад(ЕстьСкидки, ЕстьНДС И ПоказыватьНДС, ЕстьРНПТ, НазванияОбластейКолонок);
		...
		ОбластьСклад = Макет.ПолучитьОбласть(НазванияОбластейСтрок.ШапкаТаблицы + "|" + НазванияОбластейКолонок.Склад);
		...
		ТабличныйДокумент.Присоединить(ОбластьСклад);
		...
		ОбластьСкладСтандарт 					= Макет.ПолучитьОбласть(НазванияОбластейСтрок.СтрокаТаблицы
		+ "|" + НазванияОбластейКолонок.Склад);
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Иначе
				...
				ОбластьСкладСтроки 				= ОбластьСкладСтандарт;
			...
			//#Вставка
			    СтруктураДанныхСклад = Новый Структура("Склад", СкладВДокументе);
			    ОбластьСкладСтроки.Параметры.Заполнить(СтруктураДанныхСклад);
			...
			ТабличныйДокумент.Присоединить(ОбластьСкладСтроки);
...
Функция ДобавитьКолонкуСклад(СоСкидкой, СНДС, РНПТ, НазванияОбластейКолонок)
	Если СоСкидкой
		И СНДС
		И Не РНПТ Тогда
		
		НазванияОбластейКолонок.Вставить("Склад", "Склад"); 
	...
	Возврат НазванияОбластейКолонок;
